...
def main():
    ...
    tsv = csv.reader(open('IMBDW.db'), delimiter = '\t')
    for f, _, w in tsv:
        Obs[f] = w

    G = {}

    tsv = csv.reader(open('IMBD.db'), delimiter = '\t')
    p = {}
    for a, f, _ in tsv:
        if f not in p:
            p[f] = []
        for a2 in p[f]:
            Graph.makeNDLinkW(G, a, a2, Obs[f])
        p[f].append(a)
    ...
    for a1, a2 in test:
        ...
        while a1 != i:
            ...
            i = p[i]
        '''
        print (a1, a2), ':', test[(a1, a2)], ':', str(r) + ',', d[a1]
