...
def find_best_flights(flights, origin, destination):
    G = createGraph(flights)
    heap = []
    Heap.heappush(heap, (0, 0, 0, [], [origin]))
    while heap != []:
        fcost, fduration, farrival, flights, path = Heap.heappop(heap)
        w = path[-1]
        if w == destination:
            return flights
        for x in G[w]:
            for flight in G[w][x]:
                cost, departure, arrival, _ = G[w][x][flight]
                if departure > farrival:
                    new_cost = cost + fcost
                    if fduration == 0:
                        new_durn = arrival - departure
                    else:
                        new_durn = fduration + (arrival - farrival)
                    new_path = path + [x]
                    new_flights = flights + [flight]
                    Heap.heappush(heap, (new_cost, new_durn, arrival, new_flights, new_path))
    return None

def createGraph(data):
    G = {}
    for flight in data:
        (n, C1, C2, departure, arrival, cost) = flight
        val = map(int, departure.split(':'))
        departure2 = val[0] * 60 + val[1]
        val = map(int, arrival.split(':'))
        arrival2 = val[0] * 60 + val[1]
        makeDLink(G, C1, C2, n, (cost, departure2, arrival2, arrival2 - departure2))
    return G
